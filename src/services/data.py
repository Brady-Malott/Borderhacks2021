...
BICYCLE_SCORE = 1  # value of bicycle
MOTORIZED_VEHICLE_SCORE = 1  # value of motorized vehicles
LIGHT_SCORE = 2  # value of light vehicles
WORK_VAN_SCORE = 2  # value of work vans
SINGLE_UNIT_SCORE = 3  # value of single unit trucks
BUS_SCORE = 4  # value of busses
ARTICULATED_TRUCK_SCORE = 5  # value of articulated trucks
...
def traffic_score_calc(vehicle, quantity):
    ...
    if vehicle == 'Bicycle':
        ...
        return ARTICULATED_TRUCK_SCORE * quantity
    else:   
...
def _query_traffic_data(year, month, day, start_hour, start_minute):
    ...
    for intersection_id in range(1, 4):
        link = f"https://opendata.citywindsor.ca/api/traffic?date={year}-{month}-{day}" \
               f"&intersectionId={intersection_id}&start_time={start_time}&end_time={end_time}"
...
def get_traffic_data_results(direction_headed, date):
    """Entry point called from form.py.
    Takes in the direction and date, and returns the traffic score at each intersection and some other info"""
